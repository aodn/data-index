...
from data_index.testing import get_batch_filtered
import polars
...
def pipeline(
    ...
    load(
        ...
        structured_sink=StructuredParquetSink(
            path=pathlib.Path(".load/structured_metadata")
            / pathlib.Path(f"prefix={prefix.removeprefix('IMOS/')}")
            / "0.parquet"
        ),
        unstructured_sink=UnstructuredParquetSink(
            path=pathlib.Path(".load/unstructured_metadata")
            / pathlib.Path(f"prefix={prefix.removeprefix('IMOS/')}")
            / "0.parquet"
        ),
    )
